...
class Constants(BaseConstants):
	...
	indicadora_planificador_central = False
...
class Group(BaseGroup):
	...
	def recordar_tratamientos(self):
		...
		for p in self.get_players():
			self.tratamiento = int(p.participant.vars['tratamiento'])
	...
	def calcular_total_esfuerzos(self):
		...
		for jugador in jugadores:
			self.total_esfuerzos = self.total_esfuerzos + jugador.esfuerzo
	...
	def se_repite_ronda(self, stock_anterior):
		...
		if self.subsession.round_number >= Constants.num_rounds:
			...
			for ju in self.get_players():
				ju.ajustar_extracciones_individuales(temp_extraido, stock_anterior)
	...
	def calcular_crecimiento(self, p_stock):
		...
		#return round((Constants.tasa_crecimiento_intrinseca_r*int(p_stock))-(Constants.tasa_crecimiento_intrinseca_r/Constants.capacidad_de_carga_recurso_K)*(int(p_stock))**2)
	...
	def calcular_nuevo_stock(self):
		...
		if self.extraccion_agregada > int(self.stock):
			...
			for p in self.get_players():
				p.stock_registro_del_jugador = self.stock
...
class Player(BasePlayer):
	...
	def reporten_barcos(self):
		...
		for n1 in range(len(self.participant.vars['barcos_disponibles'])):
			...
			print('	[Barcos '+str(n1)+']: Inicializado con extracción '+str(barco_tem.extraccion)+', y envio: '+str(barco_tem.fue_enviado))
	...
	def calcular_extracciones_individuales(self, p_tratamiento):
		self.extracciones = 0
		lista_barcos = self.participant.vars['barcos_disponibles']
		print(' ')
		print(' ')
		print('[Participante '+str(self.participant.id_in_session)+']: '+'RONDA: '+str(self.group.subsession.round_number)+' Daré la orden a '+str(self.esfuerzo)+' barcos. Ajoy!')
	...
	def ajustar_extracciones_individuales(self, total_extraido, stock):
		...
		self.calcular_pagos_individuales()
	...
	def calcular_stock_actual(self):
		if self.group.subsession.round_number != 1:
			...
			stock_anterior = int(jugador_anterior.group.stock)
			# Y lo asigno
			self.group.stock = stock_anterior
		else:
			pass
